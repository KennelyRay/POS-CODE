#!/usr/bin/env python3
"""
Simplified Thermal Receipt Printer for Ken-dal Store POS System
Works without Pillow/QR codes for Python 3.13 compatibility
"""
...
class SimpleThermalPrinter:
    """Simplified thermal printer manager (no QR codes/images)"""
    ...
    def print_receipt(self, receipt_data: Dict[str, Any]) -> bool:
        """Print receipt with enhanced thermal quality (no QR codes)"""
        if not self.printer:
        ...
        try:
            ...
            for item in items:
                ...
                qty_price = f"  {quantity} x P{price:.2f}"
                total_str = f"P{line_total:.2f}"
            ...
            self.printer.text("Receipt ID: " + receipt_id[-8:] + "\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Simple Thermal Receipt Printer for Ken-dal Store POS')
    ...
    thermal_printer = SimpleThermalPrinter()
